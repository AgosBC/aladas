...
public class MercadoPagoService {
  ...
  public PagoReservaResponse generarPreferenciaParaReserva(Reserva reserva) {
    ...
    HttpResponse<JsonNode> response = Unirest.request("POST", urlAPI) // Patron de diseño builder que construe paso a
                                                                      // paso un request
        .header("content-type", "application/json").body(preferencia).asJson();
  ...
  public JSONObject generarPreferencia(Reserva reserva) {
    ...
    joItem.put("title", "reserva #" + reserva.getReservaId()); // este tendra items, el primero de la docu + otros como
                                                               // el tipo de moneda
    joItem.put("descripcion", "reserva generada desde aladas");// en el nodo descripcion pone el valor reserva generada
                                                               // desde aladas
    ...
    joPreference.put("external_id", reserva.getEstadoReservaId());//, "RESERVA" +reserva.getReservaId()); toda info que puede identificar algo nuestro
                                                                        // dentro de MP
    // joPreference.put("auto_return", "all"); //Redirige al cliente. Para ponerlo
    // debo indicar cuales son las url de retorno (de error, success etc )
